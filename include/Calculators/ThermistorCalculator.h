...
inline constexpr double Calculate_r_inf(const double R0, const double BFactor, const double temp0_celsius) {
  return R0 * Utilities::exp(-BFactor/(CelsiusToKelvin(temp0_celsius)));
}

inline constexpr int32_t Calculate_r_inf_micro_ohms(double R0, double BFactor, double temp0_celsius) {
  return Calculator<int32_t>::TranslateToMicro<double>(Calculate_r_inf(R0, BFactor, temp0_celsius));
}

inline constexpr double TemperatureFromResistance(double resistance, double BFactor, double r_inf) {
  return BFactor/Utilities::log(resistance/r_inf);
}

inline constexpr double ResistanceFromTemperature(double kelvin, double BFactor, double r_inf) {
...
static_assert(TemperatureFromResistance(10000, kBFactorTest, kRInfTest) <= CelsiusToKelvin(25.01), "");
static_assert(TemperatureFromResistance(10000, kBFactorTest, kRInfTest) >= CelsiusToKelvin(24.99), "");
