...
class FiniteDifferenceTester : public SecFiniteDif {
 public:
  ...
  explicit FiniteDifferenceTester(std::size_t boxcar_length)
      : SecFiniteDif{boxcar_length} {}
};
...
TEST(FiniteDifferencer, LinearValues) {
  ...
  constexpr int fWVal =
      GetAveIncreaseLineValue(start, boxcarLen * step, boxcarLen);
  constexpr int sWVal = GetAveIncreaseLineValue(
      start + boxcarLen * step, boxcarLen * step * 2, boxcarLen);
  constexpr int tWVal = GetAveIncreaseLineValue(
      start + boxcarLen * step * 2, boxcarLen * step * 3, boxcarLen);
  ...
  EXPECT_EQ(sdiff.get_sfdiff(),
            0);  //  Entering a line, so the second diff should be 0
}
...
TEST(FiniteDifferencer, ParabolicValues) {
  ...
  static_assert(getparabola(10, gain, offset) == offset + 100 * gain,
                "getparabola");
  ...
  EXPECT_EQ(2 * gain * boxcarLen * boxcarLen * boxcarLen,
            sdiff.get_sfdiff());  //  Entering a parabola, so the second diff
                                  //  should be the gain
}
