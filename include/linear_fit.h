...
inline int fit_linear(const type_t *x, const type_t *y, const size_t n,
                      std::array<type_t, 6> *p_res) {
  /*
   * Fit return is through the tuple res:
   * c0: offset
   * c1: slope
   * cov_00:
   * cov_01:
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    m_dxdy +=
        (dx * dy - m_dxdy) / (static_cast<float_type>(i) + float_type{1.0});
  }
...

  {
    ...
    *c1 = b;
    ...
    const float_type s2 =
        d2 / (static_cast<float_type>(
                 degrees_of_freedom)); /* chisq per degree of freedom */

    *cov_00 = s2 * (float_type{1.0} / static_cast<float_type>(n)) *
              (float_type{1.0} + m_x * m_x / m_dx2);
...
template <typename type_t, typename float_type = float_t>
inline int fit_linear_evenly_spaced(const size_t xstart, const type_t *y,
                                    const size_t n, type_t *c0, type_t *c1,
   ...
   * */
  for (size_t i = 0; i < n; i++) {
    m_x += (xstart + i - m_x) / (static_cast<float_type>(i) + float_type{1.0});
    m_y += (y[i] - m_y) / (static_cast<float_type>(i) + float_type{1.0});
  }
