...
template <typename T,
          bool kIsFundemental = std::is_trivially_copyable<T>::value>
class Serializer {
 public:
  static const constexpr size_t kStep = 0;
  static constexpr size_t serialize(uint8_t* storage,
                                    const size_t storage_length,
                                    const T& data) {
    assert(0);
    return 0;
  }
  static constexpr size_t deserialize(const uint8_t* storage,
                                      const size_t storage_length, T* data) {
    assert(0);
    return 0;
  }
};
...
template <typename T>
void write_as_type(uint8_t* storage, const size_t storage_length, const T* data,
...
template <typename T>
void read_as_type(const uint8_t* storage, const size_t storage_length, T* data,
                  const size_t data_length) {
  if (data_length * Serializer<T>::kStep < storage_length) {
    ...
    for (size_t i = 0; i < data_length; i++) {
      const size_t offset = Serializer<T>::kStep * i;
      Serializer<T>::deserialize(&(storage[offset]), storage_length,
                                 &(data[i]));
    }
  }
}
